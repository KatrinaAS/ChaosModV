...
#include "StreamVoting.h"

#define BUFFER_SIZE 256
#define VOTING_PROXY_START_ARGS LPSTR("chaosmod\\VotingProxy.exe --startProxy")

StreamVoting::StreamVoting(const std::array<BYTE, 3>& rgTextColor) : m_rgTextColor(rgTextColor)
{
	m_bEnableVoting = g_OptionsManager.GetStreamValue<bool>("EnableVoting", OPTION_DEFAULT_STREAM_VOTING_ENABLED);
	...
	{
		ErrorOutWithMsg("You need at least 3 enabled effects to enable Stream voting. Reverting to normal mode.");

		return;
	}

	m_iStreamSecsBeforeVoting = g_OptionsManager.GetStreamValue<int>("SecsBeforeVoting", OPTION_DEFAULT_STREAM_SECS_BEFORE_VOTING);

	m_eStreamOverlayMode = g_OptionsManager.GetStreamValue<EStreamOverlayMode>("VotingOverlayMode", static_cast<EStreamOverlayMode>(OPTION_DEFAULT_STREAM_OVERLAY_MODE));

	m_bEnableStreamChanceSystem = g_OptionsManager.GetStreamValue<bool>("VotingChanceSystem", OPTION_DEFAULT_STREAM_PROPORTIONAL_VOTING);
	m_bEnableVotingChanceSystemRetainChance = g_OptionsManager.GetStreamValue<bool>("ChanceSystemRetainChance", OPTION_DEFAULT_STREAM_PROPORTIONAL_VOTING_RETAIN_CHANCE);

	m_bEnableStreamRandomEffectVoteable = g_OptionsManager.GetStreamValue<bool>("RandomEffectVoteableEnable", OPTION_DEFAULT_STREAM_RANDOM_EFFECT);
	...
	m_hPipeHandle = CreateNamedPipe("\\\\.\\pipe\\ChaosModVChatPipe", PIPE_ACCESS_DUPLEX, PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE | PIPE_NOWAIT,
...
void StreamVoting::Run()
{
	...
	if (m_ullLastVotesFetchTime < ullCurTick - 500)
	{
		...
		if (m_bIsVotingRunning
			&& m_bEnableStreamChanceSystem
			&& !m_bEnableStreamPollVoting
			&& m_eStreamOverlayMode == EStreamOverlayMode::OverlayIngame)
		{
	...
	else if (g_pEffectDispatcher->ShouldDispatchEffectNow())
	{
		...
		&& (m_iStreamSecsBeforeVoting == 0
			|| g_pEffectDispatcher->GetRemainingTimerTime() <= m_iStreamSecsBeforeVoting)
		&& m_bIsVotingRoundDone)
	{
		...
		for (int idx = 0; idx < 4; idx++)
		{
			...
			if (idx == 3)
			{
				if (m_bEnableStreamRandomEffectVoteable)
				{
				...
				if (fChosen <= fTotalWeight)
				{
					...
					pTargetChoice = std::make_unique<ChoosableEffect>(effectIdentifier, effectData.HasCustomName
						...
						!m_bAlternatedVotingRound
							? idx + 1
							: m_bEnableStreamRandomEffectVoteable
								? idx + 5
								: idx + 4
					);
					break;
		...
		&& m_eStreamOverlayMode == EStreamOverlayMode::OverlayIngame)
	{
		...
		int iTotalVotes = 0;
		if (m_bEnableStreamChanceSystem)
		{
			for (const auto& pChoosableEffect : m_rgEffectChoices)
			{
...
_NODISCARD bool StreamVoting::IsEnabled() const
{
	return m_bEnableVoting;
}

bool StreamVoting::HandleMsg(const std::string& szMsg)
{
	if (szMsg == "hello")
	{
...
void StreamVoting::SendToPipe(std::string&& szMsg)
{
	szMsg += "\n";
	WriteFile(m_hPipeHandle, szMsg.c_str(), szMsg.length(), NULL, NULL);
}

void StreamVoting::ErrorOutWithMsg(const std::string&& szMsg)
{
